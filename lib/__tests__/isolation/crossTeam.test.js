...
const CASE_ID_A = 'caseId-alpha-001';
const CASE_ID_B = 'caseId-beta-001';
            ...
            docs = docs.map((doc) => {
              ...
              if (letVars) {
                for (const [varName, fieldExpr] of Object.entries(letVars)) {
                  const field = fieldExpr.replace(/^\$/, '');
                  localVars[varName] = doc[field];
                }
              }
...
describe('Test case isolation', () => {
  ...
  describe('reads', () => {
    it('listTestCases scoped to TEAM_A/RELEASE_A returns only TEAM_A cases', async () => {
      ...
      expect(result.data.find((tc) => tc.caseId === CASE_ID_B)).toBeUndefined();
    });
...
describe('Result isolation', () => {
  ...
  describe('writes', () => {
    it('recordResult(TEAM_A, RELEASE_B, ...) throws — no result row for this team', async () => {
      ...
      await expect(
        recordResult(db, TEAM_A, RELEASE_B, CASE_ID_B, ENV, {
          status: STATUS.PENDING,
          testedBy: null,
          notes: null,
    ...
    it('recordResult(TEAM_B, RELEASE_A, ...) throws — no result row for this team', async () => {
      await expect(
        recordResult(db, TEAM_B, RELEASE_A, CASE_ID_A, ENV, {
          status: STATUS.PENDING,
          testedBy: null,
          notes: null,
          reason: 'cross-team reset attempt',
    ...
    it('generateDenseResults(TEAM_A, RELEASE_B) throws 404 — release not found for team', async () => {
      await expect(
        generateDenseResults(db, TEAM_A, RELEASE_B, [CASE_ID_A]),
      ).rejects.toMatchObject({ status: 404 });
    });
  });
});
...
describe('Assignment isolation', () => {
  ...
  describe('reads', () => {
    it('listAssignments(TEAM_A, RELEASE_A) returns only TEAM_A assignments', async () => {
      const results = await listAssignments(db, TEAM_A, {
        releaseId: RELEASE_A,
      });
      expect(results.every((a) => a.teamId === TEAM_A)).toBe(true);
      expect(results.find((a) => a.teamId === TEAM_B)).toBeUndefined();
    });

    it('listAssignments(TEAM_A, RELEASE_B) returns empty (wrong team)', async () => {
      const results = await listAssignments(db, TEAM_A, {
        releaseId: RELEASE_B,
      });
      expect(results).toHaveLength(0);
    ...
    it('getEffectiveAssignment(TEAM_A) does not return TEAM_B assignment', async () => {
      // Call with TEAM_A but use TEAM_B case/release — should return null
      const result = await getEffectiveAssignment(db, TEAM_A, {
        releaseId: RELEASE_B,
        caseId: CASE_ID_B,
        environment: ENV,
      });
  ...
  describe('writes', () => {
    it('createAssignment scopes insertion to TEAM_A — TEAM_B count unchanged', async () => {
      const before = db._store.assignments.filter(
        (a) => a.teamId === TEAM_B,
      ).length;
      ...
      const after = db._store.assignments.filter(
        (a) => a.teamId === TEAM_B,
      ).length;
    ...
    it('deleteAssignment(TEAM_A) cannot delete a TEAM_B assignment', async () => {
      const teamBAssignment = db._store.assignments.find(
        (a) => a.teamId === TEAM_B,
      );
      expect(teamBAssignment).toBeTruthy();

      // findOne with teamId = TEAM_A will not match a TEAM_B doc
      await expect(
        deleteAssignment(db, TEAM_A, teamBAssignment._id.toString()),
      ).rejects.toMatchObject({ status: 404 });

      // TEAM_B assignment still exists
      const stillExists = db._store.assignments.find(
        (a) =>
          a._id.toString() === teamBAssignment._id.toString() &&
          a.teamId === TEAM_B,
      );
...
describe('Fingerprint matching — never crosses teams', () => {
  ...
  it('fingerprint lookup in testCasesData scopes to teamId — TEAM_A does not inherit TEAM_B caseId', async () => {
    ...
    expect(teamACandidates[0].caseId).toBe(CASE_ID_A);
    expect(teamACandidates[0].caseId).not.toBe(CASE_ID_B);
  });

  it('fingerprint query with TEAM_B credentials returns TEAM_B caseId only', async () => {
    ...
    expect(teamBCandidates[0].caseId).toBe(CASE_ID_B);
    expect(teamBCandidates[0].caseId).not.toBe(CASE_ID_A);
  });
  ...
  it('two teams sharing identical fingerprint + app name do NOT share caseId lineage', () => {
    // Both teams have a case with SHARED_FINGERPRINT. Each must have its own
    // distinct caseId so lineage is never shared.
    const teamACases = db._store.testCases.filter(
      (tc) => tc.fingerprint === SHARED_FINGERPRINT,
    );

    const caseIds = teamACases.map((tc) => tc.caseId);
    const uniqueCaseIds = new Set(caseIds);

    // Two cases (one per team), both unique caseIds
    expect(teamACases).toHaveLength(2);
    expect(uniqueCaseIds.size).toBe(2);
...
describe('teamId guard — every function rejects falsy teamId', () => {
  ...
  it('listAssignments throws 400 when teamId is empty string', async () => {
    await expect(
      listAssignments(db, '', { releaseId: RELEASE_A }),
    ).rejects.toMatchObject({ status: 400 });
  });

  it('createAssignment throws 400 when teamId is empty string', async () => {
    await expect(
      createAssignment(
        db,
        '',
        { caseId: CASE_ID_A, releaseId: RELEASE_A, assignedTo: 'alice' },
        { assignedBy: 'admin' },
      ),
    ).rejects.toMatchObject({ status: 400 });
  });
  ...
  it('recordResult throws 400 when teamId is empty string', async () => {
    await expect(
      recordResult(db, '', RELEASE_A, CASE_ID_A, ENV, {
        status: STATUS.PASS,
        testedBy: 'alice',
        notes: null,
        reason: null,
