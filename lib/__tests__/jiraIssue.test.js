...
describe('buildJiraIssueDraft', () => {
  ...
  it('includes all structured sections', () => {
    const { description } = buildDraft();
    ...
    expect(description).not.toContain('TEST INFO');
    expect(description).not.toContain('Reported by:');
    expect(description).not.toContain('Test Atlas:');
  });
...
describe('draftToJiraPayload', () => {
  ...
  it('targets the project and issue type with the test-atlas label', () => {
    ...
    expect(payload.fields.labels).toEqual(['test-atlas']);
