...
describe('buildJiraIssueDraft', () => {
  it('builds the summary as [env] title — failed in release', () => {
    expect(buildDraft().summary).toBe(
      '[QA] Login persists across restarts — failed in v2.9',
    );
  });
  ...
  it('includes expected result, QA failure notes, and context lines', () => {
    const { description } = buildDraft();
    ...
    expect(description).toContain('Recorded by: Maria');
  });
...
describe('draftToJiraPayload', () => {
  ...
  it('sets the fix version when one is supplied, omits it otherwise', () => {
    const withVersion = draftToJiraPayload({
      ...args,
      fixVersionName: 'testRelease',
    });
    expect(withVersion.fields.fixVersions).toEqual([{ name: 'testRelease' }]);

    expect(draftToJiraPayload(args).fields.fixVersions).toBeUndefined();
  });
});
