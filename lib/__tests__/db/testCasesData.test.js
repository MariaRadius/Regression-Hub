...
describe('updateTestCase', () => {
  ...
  describe('R10 — expectedResult required for Pass/Fail', () => {
    it('matches existing string _id documents when the id is ObjectId-shaped', async () => {
      ...
      const testCasesCol = {
        findOne: vi.fn(async (query) =>
          query._id?.$in?.includes(stringBackedId) ? existing : null,
        ),
        updateOne: vi.fn().mockResolvedValue({ modifiedCount: 1 }),
      };
      const eventsCol = {
      ...
      const db = {
        collection: vi.fn((name) =>
          name === 'events' ? eventsCol : testCasesCol,
        ),
      };

      await expect(
        updateTestCase(db, TEAM, stringBackedId, { status: 'Pass' }),
      ).resolves.toEqual({ ok: true });

      expect(testCasesCol.updateOne.mock.calls[0][0]._id.$in).toContain(
        stringBackedId,
      );
    });
