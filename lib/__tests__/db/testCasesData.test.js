...
describe('listTestCases', () => {
  ...
  it('adds scoped search clauses for title, application, module, and assignee', async () => {
    const db = makeListMockDb();
...
describe('deleteTestCase — cascade and audit event', () => {
   ...
   * assignments (deleteMany), and events (insertOne).
   *
  ...
  function makeDeleteMockDb({ tc = null } = {}) {
    ...
    const db = {
      collection: vi.fn((name) => {
        ...
        if (name === 'assignments') return assignmentsCol;
        if (name === 'events') return eventsCol;
        return {};
      }),
    };
    return { db, testCasesCol, testResultsCol, assignmentsCol, eventsCol };
  }
  ...
  it('calls assignments.deleteMany with { teamId, tcId } filter', async () => {
    const tcOid = new ObjectId();
    const { db, assignmentsCol } = makeDeleteMockDb({
      tc: { _id: tcOid, teamId: TEAM },
    });
    await deleteTestCase(db, TEAM, tcOid.toString(), { actor: 'Bob' });
    expect(assignmentsCol.deleteMany).toHaveBeenCalledWith(
      { teamId: TEAM, tcId: tcOid.toString() },
      expect.objectContaining({ session: expect.anything() }),
    );
  });
  ...
  it('throws 404 and skips cascade deletes when the test case is not found', async () => {
    ...
    const { db, testResultsCol, assignmentsCol } = makeDeleteMockDb({
      tc: null,
    });
    ...
    expect(assignmentsCol.deleteMany).not.toHaveBeenCalled();
  });
});
