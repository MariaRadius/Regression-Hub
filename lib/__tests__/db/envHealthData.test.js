...
describe('createEnvHealthJob', () => {
  it('inserts a queued job and returns the string id', async () => {
    ...
    expect(doc.status).toBe(JOB_STATUS.QUEUED);
...
describe('setJobProcessing', () => {
  it('updates status to processing', async () => {
    ...
    expect($set.status).toBe(JOB_STATUS.PROCESSING);
  });

  it('throws 400 for invalid jobId', async () => {
    await expect(setJobProcessing(db, 'not-valid-id')).rejects.toMatchObject({
      status: 400,
    });
  });
});
...
describe('setJobCompleted', () => {
  it('stores result and sets status to completed', async () => {
    ...
    const { $set } = collections.envHealthJobs.updateOne.mock.calls[0][1];
    expect($set.status).toBe(JOB_STATUS.COMPLETED);
    expect($set.result).toEqual({ trend: [] });
  });

  it('throws 400 for invalid jobId', async () => {
    await expect(setJobCompleted(db, 'not-valid-id', {})).rejects.toMatchObject(
      { status: 400 },
    );
  });
});
...
describe('setJobFailed', () => {
  it('stores error string and sets status to failed', async () => {
    ...
    const { $set } = collections.envHealthJobs.updateOne.mock.calls[0][1];
    expect($set.status).toBe(JOB_STATUS.FAILED);
    expect($set.error).toBe('boom');
  });

  it('throws 400 for invalid jobId', async () => {
    await expect(setJobFailed(db, 'not-valid-id', 'err')).rejects.toMatchObject(
      { status: 400 },
    );
  });
});
