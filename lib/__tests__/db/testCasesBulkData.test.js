...
describe('bulkUpdateTestCases', () => {
  ...
  describe('R12 — Pass/Fail requires softwareVersionTested when provided', () => {
    it('matches existing string _id documents when ids are ObjectId-shaped', async () => {
      const db = makeMockDb({
        missingCount: 0,
      ...
      const updateQuery = col.updateMany.mock.calls[0][0];
      expect(updateQuery._id.$in).toEqual(
        expect.arrayContaining([IDS[0], new ObjectId(IDS[0])]),
      );
    });
