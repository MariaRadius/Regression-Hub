...
describe('appendAssignmentEvents', () => {
  it('is a no-op when caseIds is empty', async () => {
    collections.events = {
      insertMany: vi.fn().mockResolvedValue({ insertedCount: 0 }),
    };
  ...
  it('fans out one doc per caseId with category=assignment and releaseId/environment', async () => {
    collections.events = {
    ...
    for (const doc of docs) {
      ...
      expect(doc.releaseId).toBe(releaseId);
      expect(doc.environment).toBe(environment);
      expect(doc.assignmentId).toBe('asgn-1');
      expect(doc.assignedTo).toBe('Bob');
      expect(doc.by).toBe('Alice');
    }

    expect(docs.map((d) => d.caseId)).toEqual(['case-1', 'case-2', 'case-3']);
  });
...
describe('listEvents', () => {
  it('sorts by at desc and returns toClientDoc-mapped docs', async () => {
    ...
    const results = await listEvents(db, TEAM, { caseId: 'case-1' });
  ...
  it('throws ApiError(400) when teamId is missing', async () => {
    await expectApiError(listEvents(db, null, { caseId: 'case-1' }), {
      status: 400,
    });
  });
});
