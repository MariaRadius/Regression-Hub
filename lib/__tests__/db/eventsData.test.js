...
describe('appendAssignmentEvents', () => {
  it('is a no-op when tcIds is empty', async () => {
    collections.events = {
      insertMany: vi.fn().mockResolvedValue({ insertedCount: 0 }),
    };
  ...
  it('fans out one doc per tcId with category=assignment and releaseId/environment', async () => {
    collections.events = {
    ...
    expect(docs.map((d) => d.tcId)).toEqual(['abc001', 'abc002', 'abc003']);
  });
