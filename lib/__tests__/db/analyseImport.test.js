...
// parseWorkbookBuffer is mocked so each test feeds exact parsed rows.
const { parseWorkbookBuffer } = vi.hoisted(() => ({
  parseWorkbookBuffer: vi.fn(),
}));
vi.mock('@/utils/excelImport', () => ({ parseWorkbookBuffer }));
...
describe('analyseImport', () => {
  ...
  it('testKey found, same team, app+module match → action update', async () => {
    parseWorkbookBuffer.mockReturnValue([row({ testKey: 'LGA-0007' })]);
    ...
    const result = await analyseImport({ collection }, TEAM, OPTS);
  ...
  it('testKey belongs to a different team → reject with error message', async () => {
    parseWorkbookBuffer.mockReturnValue([row({ testKey: 'OTHER-0001' })]);
    const { collection } = createDb({
      ...releaseSeed(),
      testCases: [
        {
  ...
  it('testKey found but application or module name mismatch → reject', async () => {
    parseWorkbookBuffer.mockReturnValue([
      row({ testKey: 'LGA-0007', applicationName: 'Other App' }),
    ]);
    const { collection } = createDb({
      ...releaseSeed(),
      applications: [
  ...
  it('testKey not found in DB → fingerprint fallback with warning', async () => {
    parseWorkbookBuffer.mockReturnValue([row({ testKey: 'LGA-9999' })]);
    const { collection } = createDb({
      ...releaseSeed(),
  ...
  it('testKey not found in DB but fingerprint matches → fingerprint fallback gives update with warning', async () => {
    parseWorkbookBuffer.mockReturnValue([row({ testKey: 'LGA-9999' })]);
    const { collection } = createDb({
      ...releaseSeed(),
      applications: [
  ...
  // -------------------------------------------------------------------------
  // Semantic 5: no testKey, fingerprint matches team-wide newest-wins → update
  // -------------------------------------------------------------------------
  it('no testKey, fingerprint matches team-wide (newest-wins) → update', async () => {
    parseWorkbookBuffer.mockReturnValue([row()]);
    const { collection } = createDb({
      ...releaseSeed(),
      testCases: [
        {
    ...
    expect(result.updateCount).toBe(1);
    expect(result.rows[0].action).toBe('update');
    // Should resolve to the newest match (case-newer / LGA-0002)
  ...
  it('in-file duplicate by testKey → both rows are rejected', async () => {
    parseWorkbookBuffer.mockReturnValue([
      row({ testKey: 'LGA-0001', testCase: 'Case A' }),
      row({ testKey: 'LGA-0001', testCase: 'Case B' }),
    ]);
  ...
  it('in-file duplicate by fingerprint within same app+module → both rows are rejected', async () => {
    // Same testCase → same fingerprint; no testKey
    parseWorkbookBuffer.mockReturnValue([
      row({ testCase: 'Duplicate test case' }),
      row({ testCase: 'Duplicate test case' }),
    ]);
    const { collection } = createDb(releaseSeed());
  ...
  it('preview mapping: reject rows excluded from rows[], surfaced in errors[]; valid===errors.length===0; createCount/updateCount; proposedInitials; warnings[]', async () => {
    parseWorkbookBuffer.mockReturnValue([
      ...
      row({ testCase: 'Brand new case', applicationName: 'New App' }),
      // Row 2 — will reject (different team test key)
      row({ testKey: 'FOREIGN-001', testCase: 'Foreign case' }),
    ]);
    const { collection } = createDb({
      ...releaseSeed(),
      testCases: [
  ...
  it('new application gets a proposed initial derived from the app name', async () => {
    parseWorkbookBuffer.mockReturnValue([
      row({ applicationName: 'Super Admin Portal' }),
    ]);
    const { collection } = createDb(releaseSeed());
  ...
  it('missing release → 404', async () => {
    ...
    await expect(
      analyseImport({ collection }, TEAM, OPTS),
