...
describe('getDashboardData', () => {
  ...
  it('counts known issues as their own category, excluded from failures and pending', async () => {
    collections.testResults = {
      aggregate: vi.fn(() => ({
        toArray: vi.fn().mockResolvedValue([
          {
            summary: [{ total: 20, passed: 8, failed: 5, knownIssue: 3 }],
            byModule: [
              { _id: 'm1', total: 20, passed: 8, failed: 5, knownIssue: 3 },
            ],
            byTester: [],
            highPrioritySummary: [],
            byCriticalCase: [],
    ...
    expect(data.summary.knownIssue).toBe(3);
    expect(data.summary.failed).toBe(5); // known issues NOT folded into failures
    // pending = total − passed − failed − knownIssue = 20 − 8 − 5 − 3 = 4
    expect(data.summary.pending).toBe(4);
    expect(data.moduleGroups.Billing.knownIssue).toBe(3);
    expect(data.moduleGroups.Billing.pending).toBe(4);
  });

  it('returns failBySeverity from the byFailSeverity facet (failed grouped by priority)', async () => {
    collections.testResults = {
      aggregate: vi.fn(() => ({
        toArray: vi.fn().mockResolvedValue([
          {
            summary: [{ total: 12, passed: 3, failed: 9 }],
            byModule: [],
            byTester: [],
            highPrioritySummary: [],
            byCriticalCase: [],
    ...
    expect(data.failBySeverity).toEqual([
      { priority: 'High', failed: 5 },
      { priority: 'Medium', failed: 3 },
      { priority: 'Unspecified', failed: 1 },
    ]);
    // Slices reconcile to the summary failed count.
    const sum = data.failBySeverity.reduce((s, r) => s + r.failed, 0);
    expect(sum).toBe(data.summary.failed);
  });
