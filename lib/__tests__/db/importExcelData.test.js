...
function normaliseId(v) {
  // Allow comparisons between ObjectId and string representations.
...
function matches(doc, filter = {}) {
  return Object.entries(filter).every(([k, cond]) => {
    if (
      ...
      if ('$in' in cond) {
        const docVal = normaliseId(doc[k]);
        return cond.$in.some((v) => normaliseId(v) === docVal);
      }
      if ('$ne' in cond) return doc[k] !== cond.$ne;
    }
    return normaliseId(doc[k]) === normaliseId(cond);
  });
}
...
function createDb(seed = {}) {
  ...
  const collection = (name) => ({
    ...
    insertMany: async (docs) => {
      // Emulate the unique (teamId, releaseId, tcId, environment) index on
    ...
    bulkWrite: async (ops = []) => {
      let inserted = 0;
      let modified = 0;
      // insertedIds is keyed by op index (0-based within the full ops array),
      // matching MongoDB bulkWrite semantics for mixed insert+update arrays.
      const insertedIds = {};
      for (let opIdx = 0; opIdx < ops.length; opIdx++) {
        const op = ops[opIdx];
        if (op.insertOne) {
          // Auto-generate _id if not provided (matches MongoDB behaviour).
          const doc = { ...op.insertOne.document };
          if (doc._id == null) {
            doc._id = new ObjectId().toString();
          }
          rows(name).push(doc);
          insertedIds[opIdx] = doc._id;
          inserted++;
...
describe('commitImport', () => {
  it('valid input → mints sequential testKeys for new cases and returns counts', async () => {
    ...
    for (const tc of inserted) {
      ...
      expect(tc.testKey).toBeTruthy();
      expect(tc.testKey).toMatch(/^[A-Z0-9]{3}-\d{4}$/);
      expect(tc.fingerprint).toBeTruthy();
    }
    // Distinct testKeys + sequential serials within the one application.
    expect(new Set(inserted.map((t) => t.testKey)).size).toBe(2);
    ...
    const createEvents = store.events.filter((e) => e.action === 'create');
    expect(createEvents).toHaveLength(2);
    // Each CREATE event carries the inserted test case's _id as tcId, and the
    // build-time placeholder field must not leak into the stored event.
    for (const evt of createEvents) {
      expect(evt.tcId).toBeTruthy();
      expect(evt).not.toHaveProperty('_pendingInsertIdx');
    }
    // tcIds match the freshly inserted documents' _ids.
    const insertedIds = new Set(inserted.map((t) => t._id.toString()));
    for (const evt of createEvents) {
      expect(insertedIds.has(evt.tcId)).toBe(true);
    }
  });

  it('fingerprint match in another release → inherits testKey from prior lineage doc', async () => {
    const existing = {
      _id: 'existing-doc',
      teamId: TEAM,
      releaseId: REL_OLD,
    ...
    // No caseId field — lineage is the MongoDB _id.
