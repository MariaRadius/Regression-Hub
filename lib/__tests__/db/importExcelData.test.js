...
describe('commitImport', () => {
  it('valid input → mints caseId + sequential testKeys for new cases and returns counts', async () => {
    parseWorkbookBuffer.mockReturnValue([
      row({ testCase: 'First case' }),
      row({ testCase: 'Second case' }),
    ]);
    const { collection, store } = createDb(releaseSeed());

    const result = await commitImport({ collection }, TEAM, OPTS);
  ...
  it('testKey match belonging to another team is NOT reused (team isolation)', async () => {
    ...
    expect(mine.caseId).not.toBe('foreign-case');
  });
  ...
  it('environment not declared by the release → 400', async () => {
    ...
    await expect(
      commitImport({ collection }, TEAM, OPTS),
