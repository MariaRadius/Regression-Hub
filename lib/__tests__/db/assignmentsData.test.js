...
describe('createAssignment', () => {
  ...
  it('inserts one doc per case and emits one ASSIGN event each', async () => {
    ...
    // Mirror: testResults.updateMany must set assignedTo on all env rows
    ...
    // release-wide assignment — no environment filter
    expect(filter.environment).toBeUndefined();
  ...
  it('defaults environment to ENVIRONMENT_SENTINEL when not supplied', async () => {
    ...
    const [docs] = collections.assignments.insertMany.mock.calls[0];
    expect(docs[0].environment).toBe(ENVIRONMENT_SENTINEL);
  });
  ...
  it('mirrors assignedTo only onto the specified environment rows when env-scoped', async () => {
    await createAssignment(
      db,
      TEAM,
      {
  ...
  it('throws 400 when tcIds is missing or empty', async () => {
    ...
    await expect(
      createAssignment(
        db,
        TEAM,
        { tcIds: [], releaseId: RELEASE_ID, assignedTo: 'Alice' },
        { assignedBy: 'Bob' },
      ),
    ).rejects.toMatchObject({ status: 400 });
  });
  ...
  it('throws 400 when assignedTo is missing', async () => {
    await expect(
      createAssignment(
        db,
        TEAM,
        { tcIds: [TC_ID], releaseId: RELEASE_ID },
        { assignedBy: 'Bob' },
      ),
    ).rejects.toMatchObject({ status: 400 });
  });
...
describe('deleteAssignment', () => {
  ...
  it('clears assignedTo to null on all testResults rows for a release-wide unassign', async () => {
    await deleteAssignment(db, TEAM, ASSIGNMENT_ID, { actor: 'Bob' });
    ...
    // release-wide sentinel → no environment filter (clears all envs)
    expect(filter.environment).toBeUndefined();
