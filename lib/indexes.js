...
export async function ensureIndexes() {
  ...
  // Drop old testCases indexes missing createdAt sort field (replaced below with sort-bearing compounds)
  await db
    .collection('testCases')
  ...
  // teamSettings — queried by teamId on every dashboard/settings/versions load
  await db
  ...
  await db
    .collection('testCases')
    .createIndex({ teamId: 1, testRunId: 1, createdAt: 1 });
  await db
    .collection('testCases')
  ...
  // partial index: covers exact-match and $ne:'' queries (versionsData, exportData);
  // case-insensitive $regex in testCasesData and testCasesBulkData bypasses this index (regex cannot satisfy partialFilterExpression)
  await db
    .collection('testCases')
    .createIndex(
      { teamId: 1, softwareVersionTested: 1, createdAt: 1 },
      { partialFilterExpression: { softwareVersionTested: { $gt: '' } } },
    );
  ...
  // History array index — covers getVersionHistoryDetail aggregate, exportData history sort, deleteVersion updateMany
  await db
    .collection('testCases')
    .createIndex({ teamId: 1, 'history.version': 1, createdAt: 1 });
