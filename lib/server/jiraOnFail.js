...
import { getApplicationById } from '@/lib/db/applicationsData';
...
/**
 * Extracts the Jira credential object from team settings.
 * Each field falls back to the corresponding env var inside jiraClient when null.
 */
function jiraConfigFromSettings(settings) {
  return {
    jiraBaseUrl: settings.jiraBaseUrl || null,
    jiraEmail: settings.jiraEmail || null,
    jiraApiToken: settings.jiraApiToken || null,
  };
}
...
async function createOne(
  ...
  const storyKey = storyOverride || testCase.jiraStory;
...
export async function createIssuesForFailures(
  ...
  const jiraConfig = jiraConfigFromSettings(settings);
  if (!isJiraConfigured(jiraConfig)) return null;
  if (settings.jiraIssueMode !== JIRA_ISSUE_MODES.AUTO) return null;

  const created = [];
  const skipped = [];
  const errors = [];

  for (const { tcId, notes } of entries) {
    try {
      const testCase = await getTestCase(db, teamId, tcId);
      const app = testCase.applicationId
        ? await getApplicationById(db, teamId, testCase.applicationId)
        : null;
 ...
 * @see {@link lib/server/__tests__/jiraOnFail.test.js}
 */
export async function buildDraftsForFailures(db, teamId, { entries }) {
  const empty = { drafts: [], skipped: [] };
  const settings = await getTeamSettings(db, teamId);
  const jiraConfig = jiraConfigFromSettings(settings);
  if (!isJiraConfigured(jiraConfig)) return empty;
  if (settings.jiraIssueMode === JIRA_ISSUE_MODES.OFF) return empty;
