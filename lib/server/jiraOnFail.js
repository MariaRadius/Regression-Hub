...
async function createOne(
  db,
  teamId,
  { releaseId, environment, tcId, summary, description },
) {
  ...
  const payload = draftToJiraPayload({
    ...
    summary,
    description,
    // Optional Jira Release bucket (e.g. "testRelease") — must exist in the
    // target project.
    fixVersionName: process.env.JIRA_FIX_VERSION || undefined,
  });
  const { key, linkError } = await createFailureIssue(payload, storyKey);
...
export async function createIssuesForFailures(
  ...
  for (const { tcId, notes, testedBy } of entries) {
    try {
      ...
      const outcome = await createOne(db, teamId, {
        releaseId,
        environment,
        tcId,
 ...
 * @param {import('mongodb').Db} db
 * @param {string} teamId
 * @param {{ releaseId: string, environment: string, issues: Array<{ tcId: string, summary: string, description: string }> }} args
 * @returns {Promise<{
 *   created: Array<{ tcId: string, key: string, linkError?: string }>,
 *   skipped: Array<{ tcId: string, reason: string }>,
 *   errors: Array<{ tcId: string, error: string }>,
