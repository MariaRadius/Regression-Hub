...
vi.mock('@/lib/db/applicationsData', () => ({ getApplicationById }));
...
beforeEach(() => {
  ...
  getTeamSettings.mockResolvedValue({
    jiraIssueMode: JIRA_ISSUE_MODES.AUTO,
    jiraBaseUrl: null,
    jiraEmail: null,
    jiraApiToken: null,
  });
  getTestCase.mockResolvedValue(storyCase);
  getApplicationById.mockResolvedValue({ _id: 'app1', name: 'Super Admin' });
...
describe('createIssuesForFailures (auto mode only)', () => {
  ...
  it('passes the release jiraSprintId into the payload', async () => {
    const argsWithSprint = {
      ...baseArgs,
      release: { ...release, jiraSprintId: '42' },
    };
    await createIssuesForFailures(db, 't1', argsWithSprint);
    const [payload] = createFailureIssue.mock.calls[0];
    expect(payload.fields.customfield_10020).toBe(42);
  });
  ...
  it('creates, links, and stores the key in auto mode', async () => {
    ...
    expect(createFailureIssue).toHaveBeenCalledWith(
      ...
      'RXR-9012',
      { jiraBaseUrl: null, jiraEmail: null, jiraApiToken: null },
    );
...
describe('buildDraftsForFailures', () => {
  it('returns one editable draft per case with steps and notes baked in', async () => {
    ...
    expect(draft.summary).toBe('Super Admin: Login works');
