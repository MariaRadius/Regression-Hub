...
/**
 * Derives sprint state from Jira's customfield_10020 sprint array.
 * Returns 'active' if any sprint is active, 'inactive' if field is present
 * but no active sprint, or null if the field is absent (team doesn't use
 * sprints / classic project).
 */
function computeSprintState(sprints) {
  if (!Array.isArray(sprints)) return null;
  if (sprints.length === 0) return 'inactive';
  return sprints.some((s) => s.state === 'active') ? 'active' : 'inactive';
}

/**
 * Fetches a single issue's fields via the direct GET /rest/api/3/issue/{key}
 * endpoint. Used as a fallback when the batch JQL search returns no results —
 * the individual endpoint has looser permission requirements and is the same
 * path used by AI story generation.
 */
async function getIssueByKey(key, config) {
  const auth = Buffer.from(`${config.email}:${config.token}`).toString(
    'base64',
  );
  const res = await fetch(
    `${config.baseUrl}/rest/api/3/issue/${encodeURIComponent(key)}?fields=summary,description,updated,status,customfield_10020`,
    {
      headers: {
        Authorization: `Basic ${auth}`,
        Accept: 'application/json',
  ...
  if (!res.ok) throw new Error(jiraErrorMessage(res.status, body));
  if (!body?.key) throw new Error(`Issue not found: ${key}`);
  return {
    key: body.key,
    summary: body.fields?.summary ?? '',
    description: extractAdfText(body.fields?.description),
    updatedAt: body.fields?.updated
      ? new Date(body.fields.updated)
      : new Date(0),
    jiraStatus: body.fields?.status?.name ?? null,
    jiraSprintState: computeSprintState(body.fields?.customfield_10020),
  };
}

export async function getIssuesByKeys(keys, jiraConfig) {
  ...
  const batchResults = (body?.issues ?? []).map((issue) => ({
    key: issue.key,
    ...
    jiraSprintState: computeSprintState(issue.fields?.customfield_10020),
  }));

  // If the batch search returned fewer results than expected (e.g. due to
  // project-level search permissions), fall back to individual fetches for
  // the missing keys — the GET /issue/{key} endpoint works with narrower
  // permissions and is the same path used by AI story generation.
  const foundKeys = new Set(batchResults.map((r) => r.key));
  const missingKeys = keys.filter((k) => !foundKeys.has(k));
  if (missingKeys.length > 0) {
    const fallbacks = await Promise.allSettled(
      missingKeys.map((k) => getIssueByKey(k, config)),
    );
