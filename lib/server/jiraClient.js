...
/**
 * Fetches `updated` and `summary` for a batch of issue keys via JQL.
 * Uses POST /rest/api/3/search to avoid URL-length limits on large batches.
 *
 * Keys not found in Jira are silently omitted.
 *
 * @param {string[]} keys
 * @param {{ jiraBaseUrl?: string|null, jiraEmail?: string|null, jiraApiToken?: string|null }} [jiraConfig]
 * @returns {Promise<Array<{ key: string, summary: string, updatedAt: Date }>>}
 */
/**
 * Fetches a single issue's `updated` and `summary` fields via the direct
 * GET /rest/api/3/issue/{key} endpoint. Used as a fallback when the batch
 * JQL search returns no results — the individual endpoint has looser
 * permission requirements and is the same path used by AI story generation.
 */
async function getIssueByKey(key, config) {
  ...
  const res = await fetch(
    `${config.baseUrl}/rest/api/3/issue/${encodeURIComponent(key)}?fields=summary,description,updated`,
    {
      headers: {
        Authorization: `Basic ${auth}`,
        Accept: 'application/json',
