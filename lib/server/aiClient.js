...
- "type": "Functional Test" | "Edge Case" | "Negative Test" | "Security Test" | "Known Issue"

Use "Known Issue" only for defects or limitations explicitly mentioned in the story description or acceptance criteria that are acknowledged but not yet fixed. All other test cases should use the remaining four types.

Generate 5–8 diverse test cases covering happy path, edge cases, negative scenarios, and any known issues described in the story.`;
}
...
function buildImpactPrompt({
  ...
  return `A Jira user story may have been updated. Compare the PREVIOUS and UPDATED versions and identify which existing test cases need to change AS A RESULT OF THAT CHANGE.

## PREVIOUS STORY (last reviewed snapshot)
Summary: ${oldSummary || '(none)'}
Description: ${oldDescription || '(none)'}
Acceptance Criteria: ${oldAcceptanceCriteria || '(none)'}

## UPDATED STORY (current)
Summary: ${newSummary}
Description: ${newDescription || '(none)'}
Acceptance Criteria: ${newAcceptanceCriteria || '(none)'}

## EXISTING TEST CASES (${existingTestCases.length} total)
${caseList || '(none)'}

## HOW TO DECIDE — this is a DIFF, not an audit

**FIRST, compute the delta between PREVIOUS and UPDATED (summary, description, acceptance criteria).**
- If the PREVIOUS and UPDATED story are effectively identical (same summary, same description, same acceptance criteria — ignoring pure whitespace), there is NOTHING to change. Return empty arrays for all three lists. Do NOT flag test cases just because their content does not perfectly match the current story — only flag what changed BETWEEN the two versions.
...
**When an ACCEPTANCE CRITERION changed, was added, or was removed (i.e. the AC text differs between PREVIOUS and UPDATED):**
...
**Default is UNAFFECTED.** When in doubt, or when the relevant part of the story did not change, leave the test case out of all three lists.
