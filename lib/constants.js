 ...
 * STATUS.PASS -> 'pass', STATUS.FAIL -> 'fail', STATUS.KNOWN_ISSUE ->
 * 'known-issue', STATUS.PENDING -> 'reset'.
 *
...
/**
 * Jira issue types created on test failure, by environment:
 * Production → BUG; every other environment → TEST_ISSUE.
 * Names must match the issue types configured in Jira exactly.
 */
...
/**
 * Team-level Jira issue creation mode (admin setting).
 * OFF — never create; ASK — fail dialog shows an opt-out checkbox;
 * AUTO — always create on Fail without asking.
 */
export const JIRA_ISSUE_MODES = Object.freeze({
  OFF: 'off',
  ASK: 'ask',
  AUTO: 'auto',
});

export const JIRA_ISSUE_MODE_DEFAULT = JIRA_ISSUE_MODES.ASK;
...
// Jira story-watch sync settings
export const JIRA_STORY_SYNC_BATCH_LIMIT = 200; // max keys per JQL batch

// Jira status names that indicate a story is no longer in scope.
// When a story's status matches one of these, its linked test cases are
// surfaced for cleanup in the Generate page discard review dialog.
export const JIRA_DISCARDED_STATUSES_DEFAULT = Object.freeze([
  'Deferred',
  'Grooming',
