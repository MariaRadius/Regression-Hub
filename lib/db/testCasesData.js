...
function buildScopedPipeline({
  ...
  return [
    ...
    { $addFields: { _createdAt: { $arrayElemAt: ['$_tc.createdAt', 0] } } },
    { $sort: { _createdAt: 1 } },
    {
      $facet: {
 ...
 */
function buildUnscopedPipeline({ definitionMatch, skip, limit }) {
  return [
...
export async function listTestCases(db, teamId, filters = {}) {
  ...
  const pipeline = scoped
    ...
    : buildUnscopedPipeline({ definitionMatch, skip, limit });

  const collection = scoped ? 'testResults' : 'testCases';

  const [[facetResult], { appMap, modMap, applications, modules }] =
    await Promise.all([
      db
