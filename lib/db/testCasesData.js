...
function buildScopedSearchMatch(searchSpec) {
  ...
  const clauses = [
    ...
    { '_tcDoc.jiraStory': searchSpec.regex },
...
export async function createTestCase(db, teamId, body) {
  ...
  // Fan out result rows for the new case across all environments the release
  // declares. Known Issue cases start as Known Issue; all others start Pending.
  // Skips silently if releaseId is absent (e.g. test harness).
  if (releaseId) {
    const initialStatus =
      doc.type === 'Known Issue' ? STATUS.KNOWN_ISSUE : STATUS.PENDING;
...
export async function countTestCasesByCombo(
  db,
  teamId,
  storyKey,
  applicationId,
  ...
  if (!teamId || !storyKey || !applicationId || !moduleId) return 0;
  return db.collection('testCases').countDocuments({
    teamId,
    jiraStory: storyKey,
    applicationId,
    moduleId,
