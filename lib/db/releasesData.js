...
export async function createRelease(db, teamId, body, { actor } = {}) {
  ...
  const environments = rawEnvs?.length
    ? rawEnvs.map(normaliseEnv).filter(Boolean)
    : [...DEFAULT_ENVIRONMENTS];
  ...
  const existing = await db
    .collection('releases')
    .findOne({ teamId, name: name.trim() });
  if (existing) {
    throw new ApiError(409, `A release named "${name.trim()}" already exists`);
  }
...
export async function updateRelease(
  ...
  if ('name' in patch) {
    ...
    if (!trimmed) throw new ApiError(400, 'name cannot be blank');

    // Name uniqueness within team (exclude self)
    const conflict = await db
      .collection('releases')
      .findOne({ teamId, name: trimmed, _id: { $ne: releaseId } });
    if (conflict) {
      throw new ApiError(409, `A release named "${trimmed}" already exists`);
    }
    update.name = trimmed;
  }
