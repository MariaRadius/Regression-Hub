  ...
  try {
    await session.withTransaction(
      async () => {
        ...
        // After inserting audit records, mirror assignees onto the new release's
        // testResults rows so carried assignments are visible on the listing (§3.5).
        if (carryAssignments) {
          ...
          if (sourceAssignments.length > 0) {
            const now3 = new Date();
            const newAssignmentDocs = sourceAssignments
              .filter((sa) => tcIdMap.has(sa.tcId))
              .map(
                ({
                  _id,
                  releaseId: _srcRId,
            ...
            // Mirror carried assignees onto the new release's testResults rows.
            // Group by (tcId, environment, assignedTo) and issue one updateMany per
            // unique (environment, assignedTo) combination to avoid over-writing.
            // Simpler: iterate and update each assignment individually; the number
            // of carried assignments is expected to be small.
            for (const doc of newAssignmentDocs) {
              const { tcId: newTcId, environment: env, assignedTo } = doc;
              const resultsFilter = { teamId, releaseId, tcId: newTcId };
              if (env && env !== ENVIRONMENT_SENTINEL) {
                resultsFilter.environment = env;
              }
              await db
