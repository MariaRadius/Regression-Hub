...
// Severity slices are ordered most→least severe; anything outside the three
// known priorities (e.g. blank) sorts last.
const SEVERITY_ORDER = [PRIORITIES.HIGH, PRIORITIES.MEDIUM, PRIORITIES.LOW];

/**
 * Build donut-chart data from a summary object.
 * Entries with value === 0 are filtered out.
 *
 * @param {{ passed: number, failed: number, pending: number, knownIssue: number, total: number }} summary
...
export function buildDonutData(summary) {
  return [
    ...
    {
      name: STATUS.KNOWN_ISSUE,
      value: summary.knownIssue ?? 0,
      total: summary.total,
    },
  ].filter((d) => d.value > 0);
}
...
/**
 * Build failure-only pie data grouped by the failed case's test-case priority
 * (High/Medium/Low). Slices are ordered most→least severe; any priority outside
 * the three known values sorts last. Zero-failure buckets are excluded, and the
 * counts always reconcile to `summary.failed`.
 *
 * @param {{ priority: string, failed: number }[]} failBySeverity
 * @returns {{ name: string, priority: string, value: number }[]}
 */
export function buildFailBySeverityData(failBySeverity) {
  const rank = (p) => {
    const i = SEVERITY_ORDER.indexOf(p);
    return i === -1 ? SEVERITY_ORDER.length : i;
  };
  return failBySeverity
    .filter((s) => s.failed > 0)
    .map((s) => ({ name: s.priority, priority: s.priority, value: s.failed }))
    .sort(
      (a, b) =>
...
export function buildModuleBarData(moduleGroups) {
  return Object.entries(moduleGroups)
    ...
    .map(([name, g]) => ({
      ...
      [STATUS.KNOWN_ISSUE]: g.knownIssue ?? 0,
    }));
}
...
export function buildAppBarData(modulesByApp) {
  return Object.entries(modulesByApp)
    ...
    .slice(0, 10)
    .map(([name, app]) => {
      const knownIssue = app.knownIssue ?? 0;
      ...
      const failPct = app.total
        ...
        : 0;
      const knownIssuePct = app.total
        ? parseFloat(((knownIssue / app.total) * 100).toFixed(1))
        : 0;
      const pendPct = app.total
        ? Math.max(
            0,
            parseFloat((100 - passPct - failPct - knownIssuePct).toFixed(1)),
          )
        : 0;
      return {
        name,
        ...
        [STATUS.KNOWN_ISSUE]: knownIssuePct,
      };
    });
}
