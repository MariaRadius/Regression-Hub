 ...
 * `testResults` rows for the given environment (live store).
 *
...
export async function createAssignment(db, teamId, body, { assignedBy }) {
  ...
  if (!environment) throw new ApiError(400, 'environment is required');
  ...
  // Mirror assignee onto testResults (live store) for the specific environment.
  const resultsFilter = {
    teamId,
    releaseId,
    tcId: { $in: tcIds },
 ...
 * `testResults` rows for the assignment's environment (live store). Emits an
 * UNASSIGN audit event.
 *
...
export async function deleteAssignment(db, teamId, id, { actor } = {}) {
  ...
  // Clear assignedTo on testResults (live store) for the assignment's environment.
  const { releaseId, tcId, environment } = assignment;
  const resultsFilter = { teamId, releaseId, tcId, environment };
  await db
    .collection('testResults')
