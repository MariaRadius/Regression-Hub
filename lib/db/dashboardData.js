...
export async function getDashboardData(
        ...
        },
        {
          $facet: {
            ...
            byFailSeverity: [
              ...
              { $group: { _id: '$priority', failed: { $sum: 1 } } },
            ],
            byCriticalCase: [
  ...
  // Failure counts by the failed case's test-case priority (High/Medium/Low).
  // Blank/absent priorities are surfaced as 'Unspecified' so the slices always
  // reconcile to `summary.failed`.
  const failBySeverity = (agg?.byFailSeverity ?? []).map((row) => ({
    priority: row._id || 'Unspecified',
    failed: row.failed,
  }));
